...
- Default cloud server: `autodl-dehaze` unless the user explicitly specifies
  `runyun-ts` or another server.
- Secondary/legacy cloud server: `runyun-ts`. Keep its details below for
  checking older runs and salvage, but do not treat it as first choice unless
  requested.
...
Validated hardware on 2026-05-27:
...
```text
torch 2.11.0+cu128 cuda 12.8 True
...
Validated data and weights on 2026-05-27:
...
## Legacy Runyun Server Role

- Role: previous/secondary training and evaluation server.
