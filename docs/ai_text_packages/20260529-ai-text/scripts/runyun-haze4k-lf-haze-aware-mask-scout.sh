...
CODE_DIR="$ROOT/code"
MODEL_NAME="${MODEL_NAME:-DEA-Net-LF-HazeAwareMask-H4K-scout-$(date +%Y%m%d-%H%M%S)}"
...
if [[ "$FAIR_PROTOCOL" -ne 1 && "${ALLOW_NONFAIR_PROTOCOL:-0}" != "1" ]]; then
  cat >&2 <<EOF
ERROR: Refusing to launch a non-fair HAZE4K Haze-Aware LF run.
...
/opt/anaconda/envs/py310/bin/python train.py \
  ...
  --lf_conditional_mask \
  --lf_haze_aware_mask \
  --lf_haze_mask_strength "${LF_HAZE_MASK_STRENGTH:-1.0}" \
