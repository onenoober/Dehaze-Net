...
CODE_DIR="$ROOT/code"
MODEL_NAME="${MODEL_NAME:-DEA-Net-LF-ResidualDirLoss-w${W_LOSS_RESIDUAL_DIR:-0.005}-H4K-scout-$(date +%Y%m%d-%H%M%S)}"
...
W_LOSS_RESIDUAL_DIR_VALUE="${W_LOSS_RESIDUAL_DIR:-0.005}"
RESIDUAL_DIR_POOL_VALUE="${RESIDUAL_DIR_POOL:-8}"
RESIDUAL_DIR_WARMUP_STEPS_VALUE="${RESIDUAL_DIR_WARMUP_STEPS:-0}"
RESIDUAL_DIR_TARGET_NORM_FLOOR_VALUE="${RESIDUAL_DIR_TARGET_NORM_FLOOR:-0.0}"
...
if [[ "$FAIR_PROTOCOL" -ne 1 && "${ALLOW_NONFAIR_PROTOCOL:-0}" != "1" ]]; then
  cat >&2 <<EOF
ERROR: Refusing to launch a non-fair HAZE4K ResidualDirLoss LF run.
...
echo "W_LOSS_RESIDUAL_DIR=$W_LOSS_RESIDUAL_DIR_VALUE"
echo "RESIDUAL_DIR_POOL=$RESIDUAL_DIR_POOL_VALUE"
echo "RESIDUAL_DIR_WARMUP_STEPS=$RESIDUAL_DIR_WARMUP_STEPS_VALUE"
...
/opt/anaconda/envs/py310/bin/python train.py \
  ...
  ${LF_EXTRA_ARGS:-} \
  --w_loss_residual_dir "$W_LOSS_RESIDUAL_DIR_VALUE" \
  --residual_dir_pool "$RESIDUAL_DIR_POOL_VALUE" \
  --residual_dir_warmup_steps "$RESIDUAL_DIR_WARMUP_STEPS_VALUE" \
  --residual_dir_target_norm_floor "$RESIDUAL_DIR_TARGET_NORM_FLOOR_VALUE" \
