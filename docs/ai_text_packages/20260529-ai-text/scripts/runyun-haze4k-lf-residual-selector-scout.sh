...
CODE_DIR="$ROOT/code"
MODEL_NAME="${MODEL_NAME:-DEA-Net-LF-ResidualSelector-H4K-scout-$(date +%Y%m%d-%H%M%S)}"
...
if [[ "$FAIR_PROTOCOL" -ne 1 && "${ALLOW_NONFAIR_PROTOCOL:-0}" != "1" ]]; then
  cat >&2 <<EOF
ERROR: Refusing to launch a non-fair HAZE4K ResidualSelector LF run.
...
/opt/anaconda/envs/py310/bin/python train.py \
  ...
  --lf_residual_selector \
  --lf_selector_hidden_channels "${LF_SELECTOR_HIDDEN_CHANNELS:-8}" \
  --lf_selector_init_bias "${LF_SELECTOR_INIT_BIAS:-2.0}" \
  ${LF_EXTRA_ARGS:-} \
