...
CODE_DIR="$ROOT/code"
MODEL_NAME="${MODEL_NAME:-DEA-Net-LFCR-v2-decay-H4K-scout100k-$(date +%Y%m%d-%H%M%S)}"
...
W_LOSS_CRPLUS_V2_VALUE="${W_LOSS_CRPLUS_V2:-0.005}"
CRPLUS_V2_WEIGHT_SCHEDULE_VALUE="${CRPLUS_V2_WEIGHT_SCHEDULE:-linear_decay}"
CRPLUS_V2_WEIGHT_DECAY_START_STEP_VALUE="${CRPLUS_V2_WEIGHT_DECAY_START_STEP:-10000}"
CRPLUS_V2_WEIGHT_DECAY_END_STEP_VALUE="${CRPLUS_V2_WEIGHT_DECAY_END_STEP:-20000}"
CRPLUS_V2_MIN_WEIGHT_VALUE="${CRPLUS_V2_MIN_WEIGHT:-0.0}"
...
if [[ "$FAIR_PROTOCOL" -ne 1 && "${ALLOW_NONFAIR_PROTOCOL:-0}" != "1" ]]; then
  cat >&2 <<EOF
ERROR: Refusing to launch a non-fair HAZE4K LFCR-v2 decay run.
...
echo "CRPLUS_V2_WEIGHT_SCHEDULE=$CRPLUS_V2_WEIGHT_SCHEDULE_VALUE"
echo "CRPLUS_V2_WEIGHT_DECAY_START_STEP=$CRPLUS_V2_WEIGHT_DECAY_START_STEP_VALUE"
echo "CRPLUS_V2_WEIGHT_DECAY_END_STEP=$CRPLUS_V2_WEIGHT_DECAY_END_STEP_VALUE"
echo "CRPLUS_V2_MIN_WEIGHT=$CRPLUS_V2_MIN_WEIGHT_VALUE"
...
"$PYTHON_BIN" train.py \
  ...
  --crplus_v2_weight_schedule "$CRPLUS_V2_WEIGHT_SCHEDULE_VALUE" \
  --crplus_v2_weight_decay_start_step "$CRPLUS_V2_WEIGHT_DECAY_START_STEP_VALUE" \
  --crplus_v2_weight_decay_end_step "$CRPLUS_V2_WEIGHT_DECAY_END_STEP_VALUE" \
  --crplus_v2_min_weight "$CRPLUS_V2_MIN_WEIGHT_VALUE" \
