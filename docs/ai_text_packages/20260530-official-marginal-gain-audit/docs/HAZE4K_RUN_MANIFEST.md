...
| `docs/ai_text_packages/20260530-cbrfrc-v1` | `experiment/dehaze-net-ai-text-pack-20260530-cbrfrc-v1` | Focused CBRFRC-v1 text package plus compact historical comparison evidence; 215 text files, about 18 MB uncompressed; implementation files, route docs, preflight CSV/JSON/logs, best-checkpoint diagnostic CSV/JSON, compact 100k training log, args, validation curve log, prior route cards, per-image route evidence, and residual diagnostics for CR/LF-v1/ResidualCalib/CRPlus-v2/LFCR-v1/LFCR-v2 comparisons; excludes checkpoints, TensorBoard, arrays, images, datasets, and the full raw 100k step log. Sync audit on 2026-05-30: source and package file sets both `215`, missing/extra `0/0`; local tracked and remote tracked package files both `215`, missing/extra remote `0/0`; forbidden tracked extensions `0`. | `COMMIT_AND_PUSH_AUDIT_REMOTE` |
| `docs/ai_text_packages/20260530-brfrc-v2-representation-audit` | `experiment/dehaze-net-ai-text-pack-20260530-brfrc-v2-representation-audit` | Focused BRFRC-v2 Stage 0 representation audit text package; 26 text files, about 713 KB uncompressed; audit code, launch script, route docs, compact AutoDL log, and small JSON/CSV/MD outputs; excludes checkpoints, TensorBoard, arrays, images, datasets, `feature_matrix_targets.npz`, and large feature tables. Sync audit on 2026-05-30: source and package file sets both `26`, missing/extra `0/0`; local tracked and remote tracked package files both `26`, missing/extra remote `0/0`; forbidden tracked extensions `0`. | `COMMIT_AND_PUSH_AUDIT_REMOTE` |
| `docs/ai_text_packages/20260530-strong-cr-abstention-snr-audit` | `experiment/dehaze-net-ai-text-pack-20260530-strong-cr-abstention-snr-audit` | Focused Strong-CR Abstention / Residual-SNR audit text package; 31 text files, about 2.1 MB uncompressed; audit code, launch script, route docs, compact AutoDL log, small JSON/CSV/MD outputs, and the CRPlus-v2 route-evidence CSV used as a text metric source. Excludes checkpoints, TensorBoard, arrays, images, datasets, and feature-matrix dumps. Sync audit on 2026-05-30: source and package file sets both `31`, missing/extra `0/0`; local tracked and remote tracked package files both `31`, missing/extra remote `0/0`; forbidden tracked extensions `0`. | `COMMIT_AND_PUSH_AUDIT_REMOTE` |
...
| `KEEP_ACTIVE_REMOTE_MODEL` | 当前云端活跃训练 | 保留完整远端 run 目录、日志和后续 checkpoint；只在 gate/final 需要时同步紧凑证据，本地不要下载大 checkpoint，除非用户明确要求 |
| `KEEP_ACTIVE_LOCAL_MODEL` | 当前本地活跃训练 | 保留完整本地 run 目录、日志和后续 checkpoint；不要加入 Git |
| `KEEP_ACTIVE_LOG` | 当前活跃 run 日志 | 保留并用于 live status/gate 检查；不要加入 Git |
...
| `DEA-Net-LF-v2-MBR-c8-H4K-scout100k-20260528-223014` | unknown remote | runyun LF-v2 MBR fair scout status unavailable | no current trusted checkpoint evidence | launched on `runyun-ts` from `/root/workspace/Dehaze-Net-audit-sync`, branch/commit `codex/haze4k-lf-v2-mbr` / `cc6b3c4`, after preflight passed; runyun later became unreachable before this result could be checked. Do not use this run as evidence unless the server becomes reachable and checkpoint/log state is verified | `KEEP_MINIMAL_REMOTE_MODEL` |
| `DEA-Net-LF-v2-MBR-c8-H4K-scout100k-autodl-20260529-144500` | stopped remote | failed 30k hard gate | 30k `29.8938 / 0.9776` | launched on `autodl-dehaze` from `/root/autodl-tmp/workspace/Dehaze-Net`, branch/commit `codex/haze4k-lf-v2-mbr` / `966d041`, after AutoDL preflight passed; stopped on 2026-05-29 because 30k quality was below CR baseline 30k and LF-v1 30k despite alive MBR branch stats and negative LF scalar gate. Do not promote or resume this exact run | `KEEP_MINIMAL_REMOTE_MODEL` |
| `DEA-Net-CBRFRC-v1-H4K-scout100k-20260530-012811` | remote model; compact local diagnostics | negative fair scout; do not promote | best step 10k `32.2237 / 0.9844`; final 100k `32.1977 / 0.9844` | launched on `autodl-dehaze` from `/root/autodl-tmp/workspace/Dehaze-Net`, branch/commit `codex/haze4k-cbrfrc-v1` / `4634b13`, after zero-gate identity, bounded-oracle, and fixed-patch micro-overfit passed under the approved safety config. Fair protocol `epochs=20`, `iters_per_epoch=5000`, total `100000`, `bs=16`, `patch_size=256`, `w_loss_L1=1.0`, `w_loss_CR=0.1`, checkpoint/eval every `10000`, frozen CR J0 with `brf_preserve_warmup_steps=3000` and `brf_preserve_gate_weight=0.0`; final diagnostics show no mechanism pass: wrong-direction `557/1000`, mean residual cosine `-0.0506`, LF MSE improved/regressed `443/557`, LF-v1 gain preservation `50.1%`; keep remote checkpoints/log, compact diagnostics synced locally; do not sync large checkpoints unless explicitly requested | `KEEP_MINIMAL_REMOTE_MODEL` |
| `HAZE4K-brfrc-v2-representation-audit-autodl-20260530-111028` | remote audit; compact local sync | failed Stage 0 diagnostic; no model training authorized | recommendation `do_not_train_brf_v2_representation_yet` | launched on `autodl-dehaze` from `/root/autodl-tmp/workspace/Dehaze-Net`, branch/commit `codex/haze4k-brfrc-v2-representation-audit` / `a441148`; used HAZE4K train split, CR and LF-v1 best checkpoints at step `90000`, and `3000` images. Internal feature ridge probes beat output-only and shuffled controls, but the best random rows failed the hard preservation gate: strong-CR preservation `0.4081` for CR features, `0.4136` for LF-v1 features, and `0.4473` for feature contrast vs required `0.70`; CR-strength held-out rows also collapsed. Keep compact text artifacts locally; keep large feature matrix/feature CSV on AutoDL only | `KEEP_SUMMARY` |
| `HAZE4K-strong-cr-abstention-snr-audit-autodl-20260530-full` | remote audit; compact local sync | failed diagnostic gate; no BRFRC-v3 training authorized | recommendation `do_not_train_abstention_brf_v3_yet` | launched on `autodl-dehaze` from `/root/autodl-tmp/workspace/Dehaze-Net`, branch/commit `codex/haze4k-strong-cr-abstention-snr-audit` / `c436abe`; used HAZE4K test split and existing CR/LF-v1/ResidualCalib/CBRFRC-v1 checkpoints, with CRPlus-v2 metrics read from committed route-evidence CSV because the checkpoint was unavailable on AutoDL. Best deployable random row `E_abstention_risk` + logistic had precision `0.6631` and simulated PSNR `+0.1174 dB` vs LF-v1, but missed strong-q4 preserve, false-intervention, LF-v1 gain preservation, and confidence-correlation gates; CR-strength strong-q4 held-out collapsed. Keep compact text artifacts locally and in the GitHub text package; no model checkpoints were generated | `KEEP_SUMMARY` |
...
| `experiment/HAZE4K/residual_field_confidence_preflight/HAZE4K-residual-field-confidence-preflight-runyun-20260528-full` | residual-field confidence continuous target preflight | compact local sync keeps `summary.json`, `summary.csv`, `split_results.csv`, and `analysis_report.md`; full feature CSV remains on runyun and should not be committed. The audit used HAZE4K train CR/LF-v1 teacher outputs (`3000` images, `12000` patches). Main random-image row had high simulated gain (`+0.8356 dB` vs LF-v1) and confidence correlation `0.3751`, but failed preserve recall (`0.6275 < 0.68`) and intervention precision (`0.5320 < 0.60`); do not launch the CR-reference residual-field 100k scout from this artifact | `KEEP_SUMMARY` |
| `experiment/HAZE4K/depth_guided_lf_preflight/HAZE4K-depth-guided-lf-preflight-autodl-20260529-204105` | Depth-Guided LF-v1 frozen-depth confidence preflight | compact local sync keeps `summary.json`, `summary.csv`, `split_results.csv`, and `analysis_report.md`; full feature CSV and depth cache remain on AutoDL and should not be committed. The audit used HAZE4K train CR/LF-v1 teacher outputs plus cached Depth Anything V2 Small relative depth maps (`3000` images, `12000` patches). Main random-image row had high simulated gain (`+0.8347 dB`) but failed preserve recall (`0.6342 < 0.68`), intervention precision (`0.5255 < 0.60`), and confidence correlation (`0.4035 < 0.45`); airlight/beta held-out rows failed, and shuffled-depth control was too close, so do not launch Depth-Guided LF-v1 training from this artifact | `KEEP_SUMMARY` |
| `experiment/HAZE4K/brfrc_v2_representation_audit/HAZE4K-brfrc-v2-representation-audit-autodl-20260530-111028` | BRFRC-v2 representation Stage 0 audit | compact local sync keeps `summary.json`, `summary.csv`, `split_results.csv`, and `analysis_report.md`; full `feature_matrix_targets.npz` and `feature_rows_compact.csv` remain on AutoDL and should not be committed. The audit used HAZE4K train split, CR/LF-v1 best checkpoints at step `90000`, and `3000` images. Internal features showed deployable signal versus output-only and shuffled controls, but failed the written pass line because strong-CR preservation stayed far below `0.70` and CR-strength held-out rows collapsed; recommendation `do_not_train_brf_v2_representation_yet` | `KEEP_SUMMARY` |
| `experiment/HAZE4K/strong_cr_abstention_snr_audit/HAZE4K-strong-cr-abstention-snr-audit-autodl-20260530-full` | Strong-CR Abstention / Residual-SNR audit | compact local sync keeps `summary.json`, `summary.csv`, `split_results.csv`, `snr_by_cr_strength.csv`, `label_rows.csv`, and `analysis_report.md`; no checkpoints or binary arrays were produced. Best deployable random row had intervention precision `0.6631` but failed strong-q4 preserve recall `0.7385 < 0.75`, strong false-intervention `0.2615 > 0.20`, LF-v1 gain preservation `0.5952 < 0.70`, and confidence correlation `0.3498 < 0.45`; CR-strength held-out strong-q4 collapsed. SNR readouts: strong-q4 residual norm ratio `0.2583`, sign flip `0.0570`; recommendation `do_not_train_abstention_brf_v3_yet` | `KEEP_SUMMARY` |
| `experiment/HAZE4K/official_marginal_gain_audit/HAZE4K-official-marginal-gain-audit-autodl-20260530-hgb-full` | Official-centric marginal gain audit | AutoDL text-only artifact with `summary.json`, `summary.csv`, `split_results.csv`, `official_candidate_matrix.csv`, `oracle_summary.json`, and `analysis_report.md`; no checkpoints, arrays, or model binaries were produced. Official oracle-with-no-change gain was `+0.6459 dB` over mean official PSNR `34.2556`, with candidate gains `>=0.15 / 0.20 dB` on `523 / 493` images, but all `448` HGB summary rows failed the deployable line. Best eligible random deployable row (`A_official_output` / `C_official_internal`, margin `0.20`) had precision `0.6597`, strong-official no-change false intervention `0.4499`, all no-change false intervention `0.6050`, and shuffled gap `0.0462`; recommendation `do_not_train_official_adapter_yet`. GitHub-readable text package created at `docs/ai_text_packages/20260530-official-marginal-gain-audit`; remote AutoDL artifact remains the source package and is worth keeping for route evidence | `KEEP_SUMMARY` |
| `experiment/HAZE4K/lf_v2_mbr_preflight/HAZE4K-lf-v2-mbr-preflight-runyun-20260528-222102` | LF-v2 MBR implementation preflight | runyun diagnostic-only preflight that passed cost, latency, neutral-init, branch activity, random backward, and HAZE4K smoke checks; static results: params overhead `0.04395%`, latency overhead `1.7766%`, neutral max diff `0.0`; smoke `smoke-H4K-LF-v2-MBR-runyun-20260528-222111` wrote step `2` and allowed the fair 100k launch | `KEEP_SUMMARY` |
| `experiment/HAZE4K/lf_v2_mbr_preflight/HAZE4K-lf-v2-mbr-preflight-autodl-` | LF-v2 MBR AutoDL implementation preflight | AutoDL diagnostic-only preflight after runyun became unreachable; passed cost, latency, neutral-init, branch activity, random backward, and HAZE4K smoke checks; static results: params overhead `0.04395%`, latency overhead `1.3762%`, neutral max diff `0.0`; smoke `smoke-H4K-LF-v2-MBR-autodl-` wrote step `2` with `lf_mbr_std_tail 0.1993409` and allowed the replacement fair 100k launch | `KEEP_SUMMARY` |
| `experiment/HAZE4K/DEA-Net-LF-v2-MBR-c8-H4K-scout100k-autodl-20260529-144500` | stopped AutoDL LF-v2 MBR fair scout | remote AutoDL run stopped at the 30k hard gate; `best.pk` and `latest.pk` are step `30000`, PSNR/SSIM `29.8938 / 0.9776`; quality was below CR baseline 30k and LF-v1 30k, with alive MBR branch stats but negative LF scalar gate, so do not promote or resume. Keep remote checkpoints/log for short-term route audit; no large local checkpoint sync was done | `KEEP_MINIMAL_REMOTE_MODEL` |
| `experiment/HAZE4K/brf_preflight/CBRFRC-v1-core-preflight-20260530` | CBRFRC-v1 identity/oracle preflight | AutoDL diagnostic-only artifact; full 1000-image zero-gate identity exactly matched frozen CR J0 (`32.2254506 / 0.9844174`, PSNR delta `0.0`, max pixel delta `0.0`), and bounded LF oracle headroom was `+10.6562 dB` with `0/250` strong-baseline oracle regressions. The same run also proved the default direct preserve gate penalty collapses the gate, so it is not the training config | `KEEP_SUMMARY` |
| `experiment/HAZE4K/brf_preflight/CBRFRC-v1-fixedmicro-warmup3000-gw0-2k-20260530` | CBRFRC-v1 approved fixed-patch micro-overfit preflight | AutoDL diagnostic-only artifact for the approved safety config `brf_preserve_warmup_steps=3000`, `brf_preserve_gate_weight=0.0`; fixed 64-image 2k micro-overfit reduced loss `0.032217 -> 0.023870`, raised residual cosine `0.0 -> 0.410177`, kept `gate_lf_mean=0.007157`, and produced nonzero applied correction `c_lf_norm=0.225575`; this passed the user safety line for launching the fair 100k scout | `KEEP_SUMMARY` |
| `experiment/HAZE4K/DEA-Net-CBRFRC-v1-H4K-scout100k-20260530-012811` | completed AutoDL CBRFRC-v1 fair scout | remote run completed the fair 100k horizon. Best checkpoint is step `10000` (`32.2237 / 0.9844`) and final checkpoint is step `100000` (`32.1977 / 0.9844`), both below CR best and LF-v1. Keep remote checkpoints/log for traceability; no large local checkpoint sync was done | `KEEP_MINIMAL_REMOTE_MODEL` |
| `experiment/HAZE4K/brf_diagnostics/DEA-Net-CBRFRC-v1-H4K-scout100k-20260530-012811-best-20260530-0830` | CBRFRC-v1 best-checkpoint full-test mechanism diagnostics | compact local sync from AutoDL, containing `summary.json`, `per_image_metrics.csv`, and `group_summary.csv` for 1000 HAZE4K test images. Key metrics: BRF best `32.2247 / 0.984419`, delta vs CR `-0.0007`, delta vs LF-v1 `-0.2035`, wrong-direction `557`, mean residual cosine `-0.0506`, LF MSE improved/regressed `443/557`, LF-v1 gain preservation `50.1%`, LF-v1 regression rescue `100%`, `gate_lf_mean=0.0327`, `gate_hf_mean=0.0072`; conclusion: residual direction mechanism failed, do not promote to Stage C | `KEEP_SUMMARY` |
| `experiment/HAZE4K/brf_diagnostics/DEA-Net-CBRFRC-v1-H4K-scout100k-20260530-012811-best-20260530-0830-route` | CBRFRC-v1 route-evidence summary | compact route summary derived from the BRF per-image CSV; duplicates the main metrics in a route-evidence-friendly summary for quick comparison | `KEEP_SUMMARY` |
| `experiment/HAZE4K/DEA-Net-OfficialWarmStart-LFv1-H4K-local-scout100k-20260528-152808` | completed local official warm-start LF-v1 staged fine-tune scout | launched 2026-05-28 from local WSL branch `codex/haze4k-official-warmstart-finetune` commit `b4194b1`; completed 2026-05-29 with best step 90k `34.2662 / 0.9886` and final 100k `34.2631 / 0.9886`; official checkpoint conversion report, `official_warmstart_step0.pk`, `best.pk`, `latest.pk`, logs, and saved metrics live here; route is isolated from cold-start fair-candidate tables | `KEEP` |
| `experiment/HAZE4K/_run_logs/DEA-Net-OfficialWarmStart-LFv1-H4K-local-scout100k-20260528-152808.log` | completed local official warm-start LF-v1 staged fine-tune log | launch/train log for the completed tmux run `ow-lfv1-local-20260528-152808`; includes official-weight mapping counts, forward-equivalence check, staged-freeze config, training progress, and final 100k gate | `KEEP_SUMMARY` |
| `experiment/HAZE4K/official_warmstart_diagnostics/DEA-Net-OfficialWarmStart-LFv1-H4K-local-scout100k-20260528-152808-best-vs-step0-20260529` | official warm-start best90k versus matched step0 per-image verification | local 1000-image test comparison; step0 `34.2548 / 0.9885`, best90k `34.2661 / 0.9886`, mean delta `+0.0114 dB`, median delta `-0.0106 dB`, better/worse `484/516`, `0.30 dB` better/worse `193/172`; keep `summary.json`, `per_image_metrics.csv`, `group_summary.csv`, `hard_cases.json`, and `analysis_report.md` | `KEEP_SUMMARY` |
...
| `experiment/HAZE4K/DEA-Net-LF-v2-MBR-c8-H4K-scout100k-20260528-222223` | small/partial remote | failed non-detached LF-v2 MBR launch; log stopped around step `487/100000`, no checkpoint/eval, no valid evidence |
| `experiment/HAZE4K/_run_logs/DEA-Net-LF-v2-MBR-c8-H4K-scout100k-20260528-222223.log` | small remote | log for failed non-detached LF-v2 MBR launch; keep only until manifest/log record is sufficient |
