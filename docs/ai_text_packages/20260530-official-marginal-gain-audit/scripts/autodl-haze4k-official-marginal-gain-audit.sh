...
RUN_ID="${RUN_ID:-HAZE4K-official-marginal-gain-audit-autodl-$(date +%Y%m%d-%H%M%S)}"
OUTPUT_DIR="${OUTPUT_DIR:-$ROOT/experiment/HAZE4K/official_marginal_gain_audit/$RUN_ID}"
...
"$PY" analyze_official_marginal_gain_audit.py \
  ...
  --target_margins "${TARGET_MARGINS:-0.15,0.20}" \
  --nochange_margin "${NOCHANGE_MARGIN:-0.05}" \
  --content_clusters "${CONTENT_CLUSTERS:-5}" \
  --heads "${HEADS:-logistic,ridge_classifier,hgb}" \
  --feature_sets "${FEATURE_SETS:-A_official_output,A_output_plus_meta,B_candidate_output_deltas,C_official_internal,D_internal_contrast,E_all_deployable,E_all_deployable_shuffled,Z_diagnostic_leakage}" \
  --min_intervention_precision "${MIN_INTERVENTION_PRECISION:-0.75}" \
  --max_strong_nochange_false_intervention "${MAX_STRONG_NOCHANGE_FALSE_INTERVENTION:-0.05}" \
  --max_nochange_false_intervention "${MAX_NOCHANGE_FALSE_INTERVENTION:-0.10}" \
  --min_mean_gain "${MIN_MEAN_GAIN:-0.02}" \
  --min_shuffled_precision_gap "${MIN_SHUFFLED_PRECISION_GAP:-0.05}"
