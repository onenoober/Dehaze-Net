...
| 10000 | Should remain close to official baseline; a small dip is acceptable only with clear recovery signs. | LF gate/stats must be finite and non-degenerate. | Stop if quality collapses or mapping/equivalence evidence is invalid. |
| 20000 | Must be recovering toward or improving over the official baseline. | Added branch must show useful, bounded activity without obvious no-op or collapse. | Stop if still below official by a large margin with no mechanism upside. |
| 30000 | First hard gate for route value. | Must show time-to-quality or branch-learning evidence that justifies more compute. | Continue only if the route is close to or above official baseline, or gives decisive evidence for a narrower next warm-start variant. |
