...
| `experiment/HAZE4K/DEA-Net-LFCR-v1-w005-H4K-scout100k-20260527-231728` | active LFCR-v1 fair scout | runyun fair 100k scout launched from `codex/haze4k-lfcr-v1` checkout commit `dbd1320`, with LFCR implementation introduced at `1a32a2a`; LF-v1 architecture plus `w_loss_crplus_v2=0.005`; 10k gate passed at `27.3004 / 0.9660`, with LF gate `0.0073388` and active `CRPlusV2` loss log; later gate decisions should use PSNR/SSIM, LF-v1 regression rescue, strong-baseline risk, residual diagnostics, LF gate activity, and CRPlus-v2 scale/frequency metrics | `PENDING_ACTIVE` |
